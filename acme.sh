...
cronac
green "域名证书申请成功或已存在！域名证书（cert.crt）和密钥（private.key）已保存到 /root/ohyca文件夹内" 
yellow "公钥文件crt路径如下，可直接复制"
green "/root/ohyca/cert.crt"
yellow "密钥文件key路径如下，可直接复制"
green "/root/ohyca/private.key"
...
green "例：原二级域名 x.ohy.eu.org 或 x.ohy.cf ，在cloudflare中重命名其中的x名称"
echo
...
installCA(){
bash ~/.acme.sh/acme.sh --install-cert -d ${ym} --key-file /root/ohyca/private.key --fullchain-file /root/ohyca/cert.crt --ecc
}
...
if [[ -n $caacme1 && ! $caacme1 == "Main_Domain" ]] && [[ -f /root/ohyca/cert.crt && -f /root/ohyca/private.key && -s /root/ohyca/cert.crt && -s /root/ohyca/private.key ]]; then
caacme=$caacme1
else
caacme='无证书申请记录'
fi
...
yellow "公钥文件crt保存路径：/root/ohyca/cert.crt"
yellow "密钥文件key保存路径：/root/ohyca/private.key"
echo
red "========================================================================="
acmeshow
