...
export sboxfiles="/etc/s-box/sbox10.json /etc/s-box/sbox11.json /etc/s-box/sbox.json"
export sboxnh=$(/etc/s-box/sing-box version 2>/dev/null | awk '/version/{print $NF}' | cut -d '.' -f 1,2)
...
if [ ! -f sbox_update ]; then
green "首次安装sbox脚本必要的依赖……"
...
argopid(){
ym=$(cat /etc/s-box/sboxargoympid.log 2>/dev/null)
ls=$(cat /etc/s-box/sboxargopid.log 2>/dev/null)
}
...
sboxcore=$(curl -Ls https://data.jsdelivr.com/v1/package/gh/SagerNet/sing-box | grep -Eo '"[0-9.]+",' | sed -n 1p | tr -d '",')
#sboxcore="1.12.5"
else
sboxcore=$(curl -Ls https://data.jsdelivr.com/v1/package/gh/SagerNet/sing-box | grep -Eo '"1\.10[0-9\.]*",'  | sed -n 1p | tr -d '",')
fi
sboxname="sing-box-$sboxcore-linux-$cpu"
curl -L -o /etc/s-box/sing-box.tar.gz  -# --retry 2 https://github.com/SagerNet/sing-box/releases/download/v$sboxcore/$sboxname.tar.gz
if [[ -f '/etc/s-box/sing-box.tar.gz' ]]; then
tar xzf /etc/s-box/sing-box.tar.gz -C /etc/s-box
mv /etc/s-box/$sboxname/sing-box /etc/s-box
rm -rf /etc/s-box/{sing-box.tar.gz,$sboxname}
...
blue "Vmess-ws将开启TLS，Hysteria-2、Tuic-v5将使用 $(cat /root/ohyca/ca.log 2>/dev/null) 证书，并开启SNI证书验证"
tlsyn=true
ym_vm_ws=$(cat /root/ohyca/ca.log 2>/dev/null)
certificatec_vmess_ws='/root/ohyca/cert.crt'
certificatep_vmess_ws='/root/ohyca/private.key'
certificatec_hy2='/root/ohyca/cert.crt'
certificatep_hy2='/root/ohyca/private.key'
certificatec_tuic='/root/ohyca/cert.crt'
certificatep_tuic='/root/ohyca/private.key'
}
...
echo
if [[ -f /root/ohyca/cert.crt && -f /root/ohyca/private.key && -s /root/ohyca/cert.crt && -s /root/ohyca/private.key ]]; then
yellow "经检测，之前已使用Acme脚本申请过Acme域名证书：$(cat /root/ohyca/ca.log) "
green "是否使用 $(cat /root/ohyca/ca.log) 域名证书？"
yellow "1：否！使用自签的证书 (回车默认)"
yellow "2：是！使用 $(cat /root/ohyca/ca.log) 域名证书"
readp "请选择【1-2】：" menu
if [ -z "$menu" ] || [ "$menu" = "1" ] ; then
zqzs
else
...
bash <(curl -Ls https://raw.githubusercontent.com/0-o0/Xplan/main/acme.sh)
if [[ ! -f /root/ohyca/cert.crt && ! -f /root/ohyca/private.key && ! -s /root/ohyca/cert.crt && ! -s /root/ohyca/private.key ]]; then
...
EOF
sboxnh=$(/etc/s-box/sing-box version 2>/dev/null | awk '/version/{print $NF}' | cut -d '.' -f 1,2)
[[ "$sboxnh" == "1.10" ]] && num=10 || num=11
cp /etc/s-box/sbox${num}.json /etc/s-box/sbox.json
}
...
command_args="run -c /etc/s-box/sbox.json"
...
ExecStart=/etc/s-box/sing-box run -c /etc/s-box/sbox.json
...
if [[ -n $($status_cmd 2>/dev/null | grep -w "$status_pattern") && -f '/etc/s-box/sbox.json' ]]; then
v4v6
if [[ -n $v4 && -n $v6 ]]; then
...
readp "请选择【1-2】：" menu
if [ -z "$menu" ] || [ "$menu" = "1" ]; then
sboxdnsip='tls://8.8.8.8/dns-query'
echo "$sboxdnsip" > /etc/s-box/sboxdnsip.log
server_ip="$v4"
echo "$server_ip" > /etc/s-box/server_ip.log
server_ipcl="$v4"
echo "$server_ipcl" > /etc/s-box/server_ipcl.log
else
sboxdnsip='tls://[2001:4860:4860::8888]/dns-query'
echo "$sboxdnsip" > /etc/s-box/sboxdnsip.log
server_ip="[$v6]"
echo "$server_ip" > /etc/s-box/server_ip.log
server_ipcl="$v6"
...
echo $ym > /root/ohyca/ca.log
fi
rm -rf /etc/s-box/vm_ws_argo.txt /etc/s-box/vm_ws.txt /etc/s-box/vm_ws_tls.txt
sboxdnsip=$(cat /etc/s-box/sboxdnsip.log)
server_ip=$(cat /etc/s-box/server_ip.log)
server_ipcl=$(cat /etc/s-box/server_ipcl.log)
uuid=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[0].users[0].uuid')
vl_port=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[0].listen_port')
vl_name=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[0].tls.server_name')
public_key=$(cat /etc/s-box/public.key)
short_id=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[0].tls.reality.short_id[0]')
argo=$(cat /etc/s-box/argo.log 2>/dev/null | grep -a trycloudflare.com | awk 'NR==2{print}' | awk -F// '{print $2}' | awk '{print $1}')
ws_path=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[1].transport.path')
vm_port=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[1].listen_port')
tls=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[1].tls.enabled')
vm_name=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[1].tls.server_name')
if [[ "$tls" = "false" ]]; then
...
hy2_port=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[2].listen_port')
...
ym=$(cat /root/ohyca/ca.log 2>/dev/null)
hy2_sniname=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[2].tls.key_path')
if [[ "$hy2_sniname" = '/etc/s-box/private.key' ]]; then
hy2_name=www.bing.com
sbox_hy2_ip=$server_ip
...
tu5_port=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[3].listen_port')
ym=$(cat /root/ohyca/ca.log 2>/dev/null)
tu5_sniname=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[3].tls.key_path')
if [[ "$tu5_sniname" = '/etc/s-box/private.key' ]]; then
tu5_name=www.bing.com
sbox_tu5_ip=$server_ip
...
argogd=$(cat /etc/s-box/sboxargoym.log 2>/dev/null)
echo
white "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
...
echo
white "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
#hy2_link="hysteria2://$uuid@$sbox_hy2_ip:$hy2_port?security=tls&alpn=h3&insecure=$ins_hy2&mport=$hyps&sni=$hy2_name#hy2-$hostname"
hy2_link="hysteria2://$uuid@$sbox_hy2_ip:$hy2_port?security=tls&alpn=h3&insecure=$ins_hy2&sni=$hy2_name#hy2-$hostname"
...
echo
white "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
tuic5_link="tuic://$uuid:$uuid@$sbox_tu5_ip:$tu5_port?congestion_control=bbr&udp_relay_mode=native&alpn=h3&sni=$tu5_name&allow_insecure=$ins&allowInsecure=$ins#tu5-$hostname"
...
#curl -L -o /etc/s-box/cloudflared -# --retry 2 https://gitlab.com/rwkgyg/sbox/-/raw/main/$cpu
...
cfargoym(){
echo
if [[ -f /etc/s-box/sboxargotoken.log && -f /etc/s-box/sboxargoym.log ]]; then
green "当前Argo固定隧道域名：$(cat /etc/s-box/sboxargoym.log 2>/dev/null)"
green "当前Argo固定隧道Token：$(cat /etc/s-box/sboxargotoken.log 2>/dev/null)"
fi
echo
...
kill -15 $(cat /etc/s-box/sboxargoympid.log 2>/dev/null) >/dev/null 2>&1
fi
echo
if [[ -n "${argotoken}" && -n "${argoym}" ]]; then
nohup setsid /etc/s-box/cloudflared tunnel --no-autoupdate --edge-ip-version auto --protocol http2 run --token ${argotoken} >/dev/null 2>&1 & echo "$!" > /etc/s-box/sboxargoympid.log
sleep 20
fi
echo ${argoym} > /etc/s-box/sboxargoym.log
echo ${argotoken} > /etc/s-box/sboxargotoken.log
crontab -l > /tmp/crontab.tmp
sed -i '/sboxargoympid/d' /tmp/crontab.tmp
echo '@reboot /bin/bash -c "nohup setsid /etc/s-box/cloudflared tunnel --no-autoupdate --edge-ip-version auto --protocol http2 run --token $(cat /etc/s-box/sboxargotoken.log 2>/dev/null) >/dev/null 2>&1 & pid=\$! && echo \$pid > /etc/s-box/sboxargoympid.log"' >> /tmp/crontab.tmp
crontab /tmp/crontab.tmp
rm /tmp/crontab.tmp
argo=$(cat /etc/s-box/sboxargoym.log 2>/dev/null)
...
if [[ -n $(ps -e | grep cloudflared) ]]; then
kill -15 $(cat /etc/s-box/sboxargopid.log 2>/dev/null) >/dev/null 2>&1
fi
nohup setsid /etc/s-box/cloudflared tunnel --url http://localhost:$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[1].listen_port') --edge-ip-version auto --no-autoupdate --protocol http2 > /etc/s-box/argo.log 2>&1 &
echo "$!" > /etc/s-box/sboxargopid.log
sleep 20
...
crontab -l > /tmp/crontab.tmp
sed -i '/sboxargopid/d' /tmp/crontab.tmp
echo '@reboot /bin/bash -c "nohup setsid /etc/s-box/cloudflared tunnel --url http://localhost:$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[1].listen_port') --edge-ip-version auto --no-autoupdate --protocol http2 > /etc/s-box/argo.log 2>&1 & pid=\$! && echo \$pid > /etc/s-box/sboxargopid.log"' >> /tmp/crontab.tmp
crontab /tmp/crontab.tmp
rm /tmp/crontab.tmp
elif [ "$menu" = "2" ]; then
kill -15 $(cat /etc/s-box/sboxargopid.log 2>/dev/null) >/dev/null 2>&1
...
sboxactive
red "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
lnsbox && blue "sbox脚本安装成功，脚本快捷方式：sbox" && cronsbox
echo
wgcfgo
sboxshare
red "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
blue "Hysteria2/Tuic5自定义V2rayN配置、Clash-Meta/Sing-box客户端配置及私有订阅链接，请选择8查看"
red "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
echo
}

changeym(){
[ -f /root/ohyca/ca.log ] && ymzs="$yellow切换为域名证书：$(cat /root/ohyca/ca.log 2>/dev/null)$plain" || ymzs="$yellow未申请域名证书，无法切换$plain"
vl_na="正在使用的域名：$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[0].tls.server_name')。$yellow更换符合reality要求的域名，不支持证书域名$plain"
tls=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[1].tls.enabled')
[[ "$tls" = "false" ]] && vm_na="当前已关闭TLS。$ymzs ${yellow}将开启TLS，Argo隧道将不支持开启${plain}" || vm_na="正在使用的域名证书：$(cat /root/ohyca/ca.log 2>/dev/null)。$yellow切换为关闭TLS，Argo隧道将可用$plain"
hy2_sniname=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[2].tls.key_path')
[[ "$hy2_sniname" = '/etc/s-box/private.key' ]] && hy2_na="正在使用自签bing证书。$ymzs" || hy2_na="正在使用的域名证书：$(cat /root/ohyca/ca.log 2>/dev/null)。$yellow切换为自签bing证书$plain"
tu5_sniname=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[3].tls.key_path')
[[ "$tu5_sniname" = '/etc/s-box/private.key' ]] && tu5_na="正在使用自签bing证书。$ymzs" || tu5_na="正在使用的域名证书：$(cat /root/ohyca/ca.log 2>/dev/null)。$yellow切换为自签bing证书$plain"
echo
green "请选择要切换证书模式的协议"
green "1：vless-reality协议，$vl_na"
if [[ -f /root/ohyca/ca.log ]]; then
...
a=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[0].tls.server_name')
b=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[0].tls.reality.handshake.server')
c=$(cat /etc/s-box/vl_reality.txt | cut -d'=' -f5 | cut -d'&' -f1)
echo $sboxfiles | xargs -n1 sed -i "23s/$a/$ym_vl_re/"
echo $sboxfiles | xargs -n1 sed -i "27s/$b/$ym_vl_re/"
restartsbox
blue "设置完毕，请回到主菜单进入选项9更新节点配置"
elif [ "$menu" = "2" ]; then
if [ -f /root/ohyca/ca.log ]; then
a=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[1].tls.enabled')
[ "$a" = "true" ] && a_a=false || a_a=true
b=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[1].tls.server_name')
[ "$b" = "www.bing.com" ] && b_b=$(cat /root/ohyca/ca.log) || b_b=$(cat /root/ohyca/ca.log)
c=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[1].tls.certificate_path')
d=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[1].tls.key_path')
if [ "$d" = '/etc/s-box/private.key' ]; then
c_c='/root/ohyca/cert.crt'
d_d='/root/ohyca/private.key'
else
c_c='/etc/s-box/cert.pem'
d_d='/etc/s-box/private.key'
fi
echo $sboxfiles | xargs -n1 sed -i "55s#$a#$a_a#"
echo $sboxfiles | xargs -n1 sed -i "56s#$b#$b_b#"
echo $sboxfiles | xargs -n1 sed -i "57s#$c#$c_c#"
echo $sboxfiles | xargs -n1 sed -i "58s#$d#$d_d#"
restartsbox
blue "设置完毕，请回到主菜单进入选项9更新节点配置"
echo
tls=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[1].tls.enabled')
...
red "当前未申请域名证书，不可切换。主菜单选择11，执行Acme证书申请" && sleep 2 && sbox
fi
elif [ "$menu" = "3" ]; then
if [ -f /root/ohyca/ca.log ]; then
c=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[2].tls.certificate_path')
d=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[2].tls.key_path')
if [ "$d" = '/etc/s-box/private.key' ]; then
c_c='/root/ohyca/cert.crt'
d_d='/root/ohyca/private.key'
else
...
echo $sboxfiles | xargs -n1 sed -i "79s#$c#$c_c#"
echo $sboxfiles | xargs -n1 sed -i "80s#$d#$d_d#"
restartsbox
blue "设置完毕，请回到主菜单进入选项9更新节点配置"
else
red "当前未申请域名证书，不可切换。主菜单选择11，执行Acme证书申请" && sleep 2 && sbox
...
if [ -f /root/ohyca/ca.log ]; then
c=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[3].tls.certificate_path')
d=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[3].tls.key_path')
if [ "$d" = '/etc/s-box/private.key' ]; then
c_c='/root/ohyca/cert.crt'
d_d='/root/ohyca/private.key'
else
...
echo $sboxfiles | xargs -n1 sed -i "102s#$c#$c_c#"
echo $sboxfiles | xargs -n1 sed -i "103s#$d#$d_d#"
restartsbox
blue "设置完毕，请回到主菜单进入选项9更新节点配置"
else
red "当前未申请域名证书，不可切换。主菜单选择11，执行Acme证书申请" && sleep 2 && sbox
...
if [ "$menu" = "1" ]; then
vlport
echo $sboxfiles | xargs -n1 sed -i "14s/$vl_port/$port_vl_re/"
restartsbox
blue "Vless-reality端口更改完成，可选择8输出配置信息"
echo
elif [ "$menu" = "2" ]; then
vmport
echo $sboxfiles | xargs -n1 sed -i "41s/$vm_port/$port_vm_ws/"
restartsbox
blue "Vmess-ws端口更改完成，可选择8输出配置信息"
tls=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[1].tls.enabled')
if [[ "$tls" = "false" ]]; then
...
hy2deports
hy2port
echo $sboxfiles | xargs -n1 sed -i "67s/$hy2_port/$port_hy2/"
restartsbox
result_vl_vm_hy_tu && reshy2 && sbox_client
else
hy2port
echo $sboxfiles | xargs -n1 sed -i "67s/$hy2_port/$port_hy2/"
restartsbox
...
readp "请选择【0-2】：" menu
if [ "$menu" = "1" ]; then
port=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[2].listen_port')
fports && result_vl_vm_hy_tu && sbox_client && changeport
elif [ "$menu" = "2" ]; then
port=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[2].listen_port')
fport && result_vl_vm_hy_tu && sbox_client && changeport
else
changeport
fi
elif [ "$menu" = "3" ]; then
if [ -n $hy2_ports ]; then
hy2deports && result_vl_vm_hy_tu && sbox_client && changeport
else
...
tu5deports
tu5port
echo $sboxfiles | xargs -n1 sed -i "89s/$tu5_port/$port_tu/"
restartsbox
result_vl_vm_hy_tu && restu5 && sbox_client
else
tu5port
echo $sboxfiles | xargs -n1 sed -i "89s/$tu5_port/$port_tu/"
restartsbox
...
readp "请选择【0-2】：" menu
if [ "$menu" = "1" ]; then
port=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[3].listen_port')
fports && result_vl_vm_hy_tu && sbox_client && changeport
elif [ "$menu" = "2" ]; then
port=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[3].listen_port')
fport && result_vl_vm_hy_tu && sbox_client && changeport
...
if [ -n $tu5_ports ]; then
tu5deports && result_vl_vm_hy_tu && sbox_client && changeport
else
...
changeuuid(){
echo
olduuid=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[0].users[0].uuid')
oldvmpath=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[1].transport.path')
...
echo $sboxfiles | xargs -n1 sed -i "s/$olduuid/$uuid/g"
restartsbox
blue "已确认uuid (密码)：${uuid}" 
blue "已确认Vmess的path路径：$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[1].transport.path')"
elif [ "$menu" = "2" ]; then
...
vmpath=$menu
echo $sboxfiles | xargs -n1 sed -i "50s#$oldvmpath#$vmpath#g"
restartsbox
fi
blue "已确认Vmess的path路径：$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[1].transport.path')"
sboxshare
...
v4v6
chip(){
rpip=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.outbounds[0].domain_strategy')
[[ "$sboxnh" == "1.10" ]] && num=10 || num=11
sed -i "111s/$rpip/$rrpip/g" /etc/s-box/sbox10.json
sed -i "134s/$rpip/$rrpip/g" /etc/s-box/sbox11.json
rm -rf /etc/s-box/sbox.json
cp /etc/s-box/sbox${num}.json /etc/s-box/sbox.json
restartsbox
}
readp "1. IPV4优先\n2. IPV6优先\n3. 仅IPV4\n4. 仅IPV6\n请选择：" choose
...
' > /etc/s-box/sboxtg.sh
sed -i "s/telegram_token/$telegram_token/g" /etc/s-box/sboxtg.sh
sed -i "s/telegram_id/$telegram_id/g" /etc/s-box/sboxtg.sh
...
tgnotice(){
if [[ -f /etc/s-box/sboxtg.sh ]]; then
green "请稍等5秒，TG机器人准备推送……"
sboxshare > /dev/null 2>&1
bash /etc/s-box/sboxtg.sh
else
yellow "未设置TG通知功能"
fi
exit
...
green "设置成功，选择主菜单8进行节点配置更新" && sleep 2 && vmesscfadd
else
changeserv
fi
}
...
[[ "$sboxnh" == "1.10" ]] && num=10 || num=11
if [[ "$sboxnh" == "1.10" ]]; then
wgipv6=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.outbounds[] | select(.type == "wireguard") | .local_address[1] | split("/")[0]')
wgprkey=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.outbounds[] | select(.type == "wireguard") | .private_key')
wgres=$(sed -n '165s/.*\[\(.*\)\].*/\1/p' /etc/s-box/sbox.json)
wgip=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.outbounds[] | select(.type == "wireguard") | .server')
wgpo=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.outbounds[] | select(.type == "wireguard") | .server_port')
else
wgipv6=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.endpoints[] | .address[1] | split("/")[0]')
wgprkey=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.endpoints[] | .private_key')
wgres=$(sed -n '125s/.*\[\(.*\)\].*/\1/p' /etc/s-box/sbox.json)
wgip=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.endpoints[] | .peers[].address')
wgpo=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.endpoints[] | .peers[].port')
fi
echo
...
sed -i "163s#$wgprkey#$menu#g" /etc/s-box/sbox10.json
sed -i "115s#$wgprkey#$menu#g" /etc/s-box/sbox11.json
readp "输入自定义IPV6地址：" menu
sed -i "161s/$wgipv6/$menu/g" /etc/s-box/sbox10.json
sed -i "113s/$wgipv6/$menu/g" /etc/s-box/sbox11.json
readp "输入自定义Reserved值 (格式：数字,数字,数字)，如无值则回车跳过：" menu
if [ -z "$menu" ]; then
menu=0,0,0
fi
sed -i "165s/$wgres/$menu/g" /etc/s-box/sbox10.json
sed -i "125s/$wgres/$menu/g" /etc/s-box/sbox11.json
rm -rf /etc/s-box/sbox.json
cp /etc/s-box/sbox${num}.json /etc/s-box/sbox.json
restartsbox
green "设置结束"
...
sed -i "157s#$wgip#$nwgip#g" /etc/s-box/sbox10.json
sed -i "158s#$wgpo#$nwgpo#g" /etc/s-box/sbox10.json
sed -i "118s#$wgip#$nwgip#g" /etc/s-box/sbox11.json
sed -i "119s#$wgpo#$nwgpo#g" /etc/s-box/sbox11.json
rm -rf /etc/s-box/sbox.json
cp /etc/s-box/sbox${num}.json /etc/s-box/sbox.json
restartsbox
...
sboxymfl(){
sboxport=$(cat /etc/s-box/sboxwpph.log 2>/dev/null | awk '{print $3}' | awk -F":" '{print $NF}') 
sboxport=${sboxport:-'40000'}
resv1=$(curl -s --socks5 localhost:$sboxport icanhazip.com)
resv2=$(curl -sx socks5h://localhost:$sboxport icanhazip.com)
...
wd4=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.route.rules[1].domain_suffix | join(" ")')
wg4=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.route.rules[1].geosite | join(" ")' 2>/dev/null)
if [[ "$wd4" == "o_o" && ("$wg4" == "o_o" || -z "$wg4") ]]; then
...
wd6=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.route.rules[2].domain_suffix | join(" ")')
wg6=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.route.rules[2].geosite | join(" ")' 2>/dev/null)
if [[ "$wd6" == "o_o" && ("$wg6" == "o_o"|| -z "$wg6") ]]; then
...
sd4=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.route.rules[3].domain_suffix | join(" ")')
sg4=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.route.rules[3].geosite | join(" ")' 2>/dev/null)
if [[ "$sd4" == "o_o" && ("$sg4" == "o_o" || -z "$sg4") ]]; then
...
sd6=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.route.rules[4].domain_suffix | join(" ")')
sg6=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.route.rules[4].geosite | join(" ")' 2>/dev/null)
if [[ "$sd6" == "o_o" && ("$sg6" == "o_o" || -z "$sg6") ]]; then
...
ad4=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.route.rules[5].domain_suffix | join(" ")')
ag4=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.route.rules[5].geosite | join(" ")' 2>/dev/null)
if [[ "$ad4" == "o_o" && ("$ag4" == "o_o" || -z "$ag4") ]]; then
...
ad6=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.route.rules[6].domain_suffix | join(" ")')
ag6=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.route.rules[6].geosite | join(" ")' 2>/dev/null)
if [[ "$ad6" == "o_o" && ("$ag6" == "o_o" || -z "$ag6") ]]; then
...
echo
[[ "$sboxnh" == "1.10" ]] && blue "当前Sing-box内核支持geosite分流方式" || blue "当前Sing-box内核不支持geosite分流方式，仅支持分流2、3、5、6选项"
echo
yellow "注意："
...
sed -i "184s/.*/$w4flym/" /etc/s-box/sbox.json /etc/s-box/sbox10.json
restartsbox
changef
elif [ "$menu" = "2" ]; then
...
sed -i "187s/.*/$w4flym/" /etc/s-box/sbox.json /etc/s-box/sbox10.json
restartsbox
changef
else
changef
...
sed -i "193s/.*/$w6flym/" /etc/s-box/sbox10.json
sed -i "169s/.*/$w6flym/" /etc/s-box/sbox11.json
sed -i "181s/.*/$w6flym/" /etc/s-box/sbox11.json
rm -rf /etc/s-box/sbox.json
cp /etc/s-box/sbox${num}.json /etc/s-box/sbox.json
restartsbox
changef
...
sed -i "196s/.*/$w6flym/" /etc/s-box/sbox.json /etc/s-box/sbox10.json
restartsbox
changef
else
...
sed -i "202s/.*/$s4flym/" /etc/s-box/sbox10.json
sed -i "162s/.*/$s4flym/" /etc/s-box/sbox11.json
sed -i "175s/.*/$s4flym/" /etc/s-box/sbox11.json
rm -rf /etc/s-box/sbox.json
cp /etc/s-box/sbox${num}.json /etc/s-box/sbox.json
restartsbox
changef
...
sed -i "205s/.*/$s4flym/" /etc/s-box/sbox.json /etc/s-box/sbox10.json
restartsbox
changef
else
yellow "遗憾！当前Sing-box内核不支持geosite分流方式。如要支持，请切换1.10系列内核" && exit
...
sed -i "211s/.*/$s6flym/" /etc/s-box/sbox.json /etc/s-box/sbox10.json
restartsbox
changef
elif [ "$menu" = "2" ]; then
...
sed -i "214s/.*/$s6flym/" /etc/s-box/sbox.json /etc/s-box/sbox10.json
restartsbox
changef
else
changef
...
sed -i "220s/.*/$ad4flym/" /etc/s-box/sbox10.json
sed -i "188s/.*/$ad4flym/" /etc/s-box/sbox11.json
rm -rf /etc/s-box/sbox.json
cp /etc/s-box/sbox${num}.json /etc/s-box/sbox.json
restartsbox
changef
...
sed -i "223s/.*/$ad4flym/" /etc/s-box/sbox.json /etc/s-box/sbox10.json
restartsbox
changef
else
yellow "遗憾！当前Sing-box内核不支持geosite分流方式。如要支持，请切换1.10系列内核" && exit
...
sed -i "229s/.*/$ad6flym/" /etc/s-box/sbox10.json
sed -i "194s/.*/$ad6flym/" /etc/s-box/sbox11.json
rm -rf /etc/s-box/sbox.json
cp /etc/s-box/sbox${num}.json /etc/s-box/sbox.json
restartsbox
changef
...
sed -i "232s/.*/$ad6flym/" /etc/s-box/sbox.json /etc/s-box/sbox10.json
restartsbox
changef
else
yellow "遗憾！当前Sing-box内核不支持geosite分流方式。如要支持，请切换1.10系列内核" && exit
...
sed -i '/sboxargopid/d' /tmp/crontab.tmp
sed -i '/sboxargoympid/d' /tmp/crontab.tmp
sed -i '/sboxwpphid.log/d' /tmp/crontab.tmp
crontab /tmp/crontab.tmp
rm /tmp/crontab.tmp
}
...
lnsbox(){
rm -rf /usr/bin/sbox
curl -L -o /usr/bin/sbox -# --retry 2 --insecure https://raw.githubusercontent.com/0-o0/Xplan/main/sbox.sh
chmod +x /usr/bin/sbox
}
...
sboxname="sing-box-$upcore-linux-$cpu"
curl -L -o /etc/s-box/sing-box.tar.gz  -# --retry 2 https://github.com/SagerNet/sing-box/releases/download/v$upcore/$sboxname.tar.gz
if [[ -f '/etc/s-box/sing-box.tar.gz' ]]; then
tar xzf /etc/s-box/sing-box.tar.gz -C /etc/s-box
mv /etc/s-box/$sboxname/sing-box /etc/s-box
rm -rf /etc/s-box/{sing-box.tar.gz,$sboxname}
...
cp /etc/s-box/sbox${num}.json /etc/s-box/sbox.json
restartsbox
blue "成功升级/切换 Sing-box 内核版本：$(/etc/s-box/sing-box version | awk '/version/{print $NF}')" && sleep 3 && sbox
else
red "下载 Sing-box 内核不完整，安装失败，请重试" && upsboxcroe
fi
else
red "下载 Sing-box 内核失败或不存在，请重试" && upsboxcroe
fi
else
red "版本号检测出错，请重试" && upsboxcroe
fi
}
...
kill -15 $(cat /etc/s-box/sboxargopid.log 2>/dev/null) >/dev/null 2>&1
kill -15 $(cat /etc/s-box/sboxargoympid.log 2>/dev/null) >/dev/null 2>&1
kill -15 $(cat /etc/s-box/sboxwpphid.log 2>/dev/null) >/dev/null 2>&1
rm -rf /etc/s-box sbox_update /usr/bin/sbox /root/geoip.db /root/geosite.db /root/warpapi /root/warpip
uncronsbox
iptables -t nat -F PREROUTING >/dev/null 2>&1
netfilter-persistent save >/dev/null 2>&1
service iptables save >/dev/null 2>&1
green "Sing-box卸载完成！"
blue "欢迎继续使用sbox脚本：bash <(curl -Ls https://raw.githubusercontent.com/0-o0/Xplan/main/sbox.sh)"
echo
}
...
red "安卓SFA、苹果SFI，win电脑官方文件包SFW请自行下载，"
...
bash <(curl -Ls https://raw.githubusercontent.com/0-o0/Xplan/main/warp.sh)
}
bbr(){
...
echo -e "🚀【 Vless-reality 】${yellow}端口:$vl_port  Reality域名证书伪装地址：$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[0].tls.server_name')${plain}"
if [[ "$tls" = "false" ]]; then
...
if [ "$argoym" = "已开启" ]; then
echo -e "Vmess-UUID：${yellow}$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[0].users[0].uuid')${plain}"
echo -e "Vmess-Path：${yellow}$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.inbounds[1].transport.path')${plain}"
if [[ -n $(ps -e | grep -w $ls 2>/dev/null) ]]; then
...
if [[ -n $(ps -e | grep -w $ym 2>/dev/null) ]]; then
echo -e "Argo固定域名：${yellow}$(cat /etc/s-box/sboxargoym.log 2>/dev/null)${plain}"
fi
fi
echo "------------------------------------------------------------------------------------"
if [[ -n $(ps -e | grep sboxwpph) ]]; then
s5port=$(cat /etc/s-box/sboxwpph.log 2>/dev/null | awk '{print $3}'| awk -F":" '{print $NF}')
s5gj=$(cat /etc/s-box/sboxwpph.log 2>/dev/null | awk '{print $6}')
case "$s5gj" in
AT) showgj="奥地利" ;;
AU) showgj="澳大利亚" ;;
BE) showgj="比利时" ;;
...
grep -q "country" /etc/s-box/sboxwpph.log 2>/dev/null && s5ms="多地区Psiphon代理模式 (端口:$s5port  国家:$showgj)" || s5ms="本地Warp代理模式 (端口:$s5port)"
...
[[ "$sboxnh" == "1.10" ]] && ymflzu=("ww4" "ww6" "ws4" "ws6" "l4" "l6") || ymflzu=("ww6" "ws4" "l4" "l6")
...
sboxactive
ins(){
if [ ! -e /etc/s-box/sboxwpph ]; then
case $(uname -m) in
aarch64) cpu=arm64;;
x86_64) cpu=amd64;;
esac
curl -L -o /etc/s-box/sboxwpph -# --retry 2 --insecure https://raw.githubusercontent.com/0-o0/Xplan/main/sboxwpph_$cpu
chmod +x /etc/s-box/sboxwpph
fi
if [[ -n $(ps -e | grep sboxwpph) ]]; then
kill -15 $(cat /etc/s-box/sboxwpphid.log 2>/dev/null) >/dev/null 2>&1
fi
...
s5port=$(sed 's://.*::g' /etc/s-box/sbox.json | jq -r '.outbounds[] | select(.type == "socks") | .server_port')
[[ "$sboxnh" == "1.10" ]] && num=10 || num=11
sed -i "127s/$s5port/$port/g" /etc/s-box/sbox10.json
sed -i "150s/$s5port/$port/g" /etc/s-box/sbox11.json
rm -rf /etc/s-box/sbox.json
cp /etc/s-box/sbox${num}.json /etc/s-box/sbox.json
restartsbox
}
unins(){
kill -15 $(cat /etc/s-box/sboxwpphid.log 2>/dev/null) >/dev/null 2>&1
rm -rf /etc/s-box/sboxwpph.log /etc/s-box/sboxwpphid.log
crontab -l > /tmp/crontab.tmp
sed -i '/sboxwpphid.log/d' /tmp/crontab.tmp
crontab /tmp/crontab.tmp
rm /tmp/crontab.tmp
...
if [ "$menu" = "1" ]; then
ins
nohup setsid /etc/s-box/sboxwpph -b 127.0.0.1:$port --gool -$sw46 --endpoint 162.159.192.1:2408 >/dev/null 2>&1 & echo "$!" > /etc/s-box/sboxwpphid.log
...
echo "/etc/s-box/sboxwpph -b 127.0.0.1:$port --gool -$sw46 --endpoint 162.159.192.1:2408 >/dev/null 2>&1" > /etc/s-box/sboxwpph.log
crontab -l > /tmp/crontab.tmp
sed -i '/sboxwpphid.log/d' /tmp/crontab.tmp
echo '@reboot /bin/bash -c "nohup setsid $(cat /etc/s-box/sboxwpph.log 2>/dev/null) & pid=\$! && echo \$pid > /etc/s-box/sboxwpphid.log"' >> /tmp/crontab.tmp
crontab /tmp/crontab.tmp
rm /tmp/crontab.tmp
...
nohup setsid /etc/s-box/sboxwpph -b 127.0.0.1:$port --cfon --country $guojia -$sw46 --endpoint 162.159.192.1:2408 >/dev/null 2>&1 & echo "$!" > /etc/s-box/sboxwpphid.log
green "申请IP中……请稍等……" && sleep 20
resv1=$(curl -s --socks5 localhost:$port icanhazip.com)
resv2=$(curl -sx socks5h://localhost:$port icanhazip.com)
if [[ -z $resv1 && -z $resv2 ]]; then
...
echo "/etc/s-box/sboxwpph -b 127.0.0.1:$port --cfon --country $guojia -$sw46 --endpoint 162.159.192.1:2408 >/dev/null 2>&1" > /etc/s-box/sboxwpph.log
crontab -l > /tmp/crontab.tmp
sed -i '/sboxwpphid.log/d' /tmp/crontab.tmp
echo '@reboot /bin/bash -c "nohup setsid $(cat /etc/s-box/sboxwpph.log 2>/dev/null) & pid=\$! && echo \$pid > /etc/s-box/sboxwpphid.log"' >> /tmp/crontab.tmp
crontab /tmp/crontab.tmp
...
clear
white "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" 
echo -e "\e[38;2;153;204;255m    ____▄▀▀▀▄▄▄▄▄▄▄▀▀▀▄\e[0m"
echo -e "\e[38;2;153;204;255m    ─────█▒▒░░░░░░░░░▒▒█\e[0m"
echo -e "\e[38;2;153;204;255m  ────────█░░█░░░░░█░░█\e[0m"
echo -e "\e[38;2;153;204;255m    ───▄▄──█░░░▀█▀░░░█──▄▄\e[0m"
echo -e "\e[38;2;153;204;255m      █░░▀─▀▄░░░░░░░▄▀─▀░░█\e[0m"
echo -e "\e[38;2;153;204;255m       █▄█ █▀█ █▀█ █▀█ █▄█\e[0m"
...
echo -e "\e[38;2;153;204;255m\"*°•.¸^ ~ ^¸.•°*\"~*°•.¸^\"*°•.¸.•°*\"~*°•.¸^\"*°•.¸\e[0m"
white "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" 
white "Vless-reality-vision、Vmess-ws(tls)+Argo、Hysteria-2、Tuic-v5 一键部署"
white "脚本快捷方式：sbox"
red "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"
...
rpip=$(sed 's://.*::g' /etc/s-box/sbox.json 2>/dev/null | jq -r '.outbounds[0].domain_strategy')
...
elif [[ -z $($status_cmd 2>/dev/null | grep -w "$status_pattern") && -f '/etc/s-box/sbox.json' ]]; then
